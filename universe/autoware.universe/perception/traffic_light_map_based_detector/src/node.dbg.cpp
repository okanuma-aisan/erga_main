...
//#define FULL_SCREEN_ROIS  // pubするROIすべてをフルスクリーン領域にする
  ...
  tf_listener_(tf_buffer_)
{
  ...
  // for Debug
  dbg_msg_pub_ =
    this->create_publisher<std_msgs::msg::String>("~/debug/msg", 1);

  std::string message = "MapBasedDetector() started";
  ...
  const rclcpp::Time & t, const std::string & frame_id, tf2::Transform & tf) const
{
  ...
  try {
    ...
    message += ("-> catch tf2::ConnectivityException (map -> " + frame_id + ")");
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
  } catch (tf2::ExtrapolationException & ex) {
    message += ("-> catch tf2::ExtrapolationException (map -> " + frame_id + ")");
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
  } catch (tf2::InvalidArgumentException & ex) {
    message += ("-> catch tf2::InvalidArgumentException (map -> " + frame_id + ")");
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
  } catch (tf2::LookupException & ex) {
    message += ("-> catch tf2::LookupException (map -> " + frame_id + ")");
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
  } catch (tf2::TimeoutException & ex) {
    message += ("-> catch tf2::TimeoutException (map -> " + frame_id + ")");
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
  } catch (tf2::TransformException & ex) {
    message += ("-> catch tf2::TransformException (map -> " + frame_id + ")");
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
    return false;
  }
...
{
  std::string message = "cameraInfoCallback()";
  std_msgs::msg::String debug_msg;

  if (all_traffic_lights_ptr_ == nullptr && route_traffic_lights_ptr_ == nullptr) {
    message += "-> all_traffic_lights_ptr_ == nullptr && route_traffic_lights_ptr_ == nullptr";
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
    return;
  }
  ...
  if (!getTransform(
    ...
    message += "-> cannot get transform from map frame to camera frame";
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
    return;
  }
...
{
  std::string message = "getTrafficLightRoi()";
  std_msgs::msg::String debug_msg;
  ...
  // for roi.width and roi.height
  {
    ...
    if (roi.roi.width < 1 || roi.roi.height < 1) {
      message += "-> roi.roi.width < 1 || roi.roi.height < 1";
      debug_msg.data = message;
      dbg_msg_pub_->publish(debug_msg);
      return false;
    }
...
#ifdef FULL_SCREEN_ROIS  // for Test
  out_roi.roi.x_offset = 0;
  out_roi.roi.y_offset = 0;
  out_roi.roi.width = pinhole_camera_model.cameraInfo().width - 1;
  out_roi.roi.height = pinhole_camera_model.cameraInfo().height - 1;
#endif
...
{
  std::string message = "routeCallback()";
  std_msgs::msg::String debug_msg;

  if (lanelet_map_ptr_ == nullptr) {
    RCLCPP_WARN(get_logger(), "cannot set traffic light in route because don't receive map");
    
    message += "-> cannot set traffic light in route because don't receive map";
    debug_msg.data = message;
    dbg_msg_pub_->publish(debug_msg);
    return;
  }
  ...
  for (const auto & segment : input_msg->segments) {
    for (const auto & primitive : segment.primitives) {
      try {
        ...
        message += "-> lanelet::NoSuchPrimitiveError";
        debug_msg.data = message;
        dbg_msg_pub_->publish(debug_msg);
        return;
      }
...
{
  std::string message = "getVisibleTrafficLights()";
  std_msgs::msg::String debug_msg;
...
{
  std::string message = "publishVisibleTrafficLights()";
  std_msgs::msg::String debug_msg;
