  ...
  const double current_velocity, const double desired_acc, bool & accel_cmd_is_zero)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("calculateAccelMap"), "desired_accel_cmd: %f", desired_accel_cmd);
  desired_accel_cmd = (desired_accel_cmd == 0.0) ? 0.0 : std::max(std::min(desired_accel_cmd, 1024.0), max_accel_cmd_);
  ...
  const double current_velocity, const double desired_acc)
{
  ...
  desired_brake_cmd = std::min(std::max(desired_brake_cmd, 1024.0), max_brake_cmd_);
