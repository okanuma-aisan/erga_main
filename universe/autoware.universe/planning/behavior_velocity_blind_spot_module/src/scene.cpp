...
#if 1    
#define PCD_TARGET_AREAS	conflict_areas //detection_areas
#define PCD_TARGET_MARGIN	(0.2)
    if (! obstacle_detected) {
      const auto & PCD_TARGET_AREAS = areas_opt.get().PCD_TARGET_AREAS;
      const auto & points = *(planner_data_->no_ground_pointcloud);
      for (const auto p : points) {
        for (const auto & area : PCD_TARGET_AREAS) {
          const auto p2 = Point2d{p.x, p.y};
          if (bg::within(p2, lanelet::utils::to2D(area))) {
            bool is_in_objects = false;
            for (const auto & object : objects.objects) {
              // exclude pedestrian, bicycle, motorcycle and unknown
              if (isTargetObjectType(object) ||
              ...
              LineString2d l1, l2;
              tf2::Transform otf;
              const auto ox = object.kinematics.initial_pose_with_covariance.pose.position.x;
              const auto oy = object.kinematics.initial_pose_with_covariance.pose.position.y;
              otf.setOrigin(tf2::Vector3(ox, oy, 0.0));
              const auto mq = object.kinematics.initial_pose_with_covariance.pose.orientation;
              tf2::Quaternion oq(mq.x, mq.y, mq.z, mq.w);
              otf.setRotation(oq);
              const auto ow = object.shape.dimensions.x + PCD_TARGET_MARGIN*2;
              const auto oh = object.shape.dimensions.y + PCD_TARGET_MARGIN*2;
              const auto ol1 = otf * tf2::Vector3(-ow/2, -oh/2, 0.0);
              const auto ol2 = otf * tf2::Vector3(-ow/2, oh/2, 0.0);
              const auto or1 = otf * tf2::Vector3(ow/2, -oh/2, 0.0);
              const auto or2 = otf * tf2::Vector3(ow/2, oh/2, 0.0);
              l1.push_back(Point2d{ol1.x(), ol1.y()});
              l1.push_back(Point2d{ol2.x(), ol2.y()});
              l2.push_back(Point2d{or1.x(), or1.y()});
              l2.push_back(Point2d{or2.x(), or2.y()});
              if (bg::within(p2, lines2polygon(l1, l2))) {
                is_in_objects = true;
                break;
              }
            }
            if (!is_in_objects) {
              RCLCPP_INFO(logger_, "detected at (%f,%f)", p.x, p.y);
              return true;
            }
          }
        }
...
/*
   // check pointcloud in blind spot areas
    if (! obstacle_detected) {
      const auto & detection_areas = areas_opt.get().detection_areas;
      const auto & points = *(planner_data_->no_ground_pointcloud);
      for (const auto p : points) {
        for (const auto & area : detection_areas) {
          if (bg::within(Point2d{p.x, p.y}, lanelet::utils::to2D(area))) {
            RCLCPP_INFO(logger_, "detected at (%f,%f)", p.x, p.y);
            return true;
          }
        }
...
*/    
    ...
    if (! obstacle_detected) {
      ...
      const auto & points = *(planner_data_->no_ground_pointcloud);
      for (const auto p : points) {
        for (const auto & area : conflict_areas) {
          if (bg::within(Point2d{p.x, p.y}, lanelet::utils::to2D(area))) {
            RCLCPP_INFO(logger_, "detected at (%f,%f)", p.x, p.y);
            return true;
          }
