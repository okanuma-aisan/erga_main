  ...
  const double dist_threshold, PredictedObjects & filtered_objects)
{
  ...
  for (auto & object : input_objects.objects) {
    ...
    const boost::optional<size_t> seg_idx_check = motion_utils::findNearestSegmentIndex(
      traj, object.kinematics.initial_pose_with_covariance.pose.position);
    //seg_idx 2024.01.17 (by )
    if ( seg_idx_check == boost::none ) continue;

    const size_t seg_idx = seg_idx_check.get();
    if ( seg_idx > traj.size() - 2 ||  traj.size() <2 )  continue;
