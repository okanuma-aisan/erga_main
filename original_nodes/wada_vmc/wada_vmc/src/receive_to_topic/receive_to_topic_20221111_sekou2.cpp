...
namespace wada_vmc
{
	...
	private://受信メッセージをros2 topic化する関数群
		...
		void publish501(const std::array<uint8_t, 8> &receive_buffer)
		{
			...
			tmp_ptr = reinterpret_cast<uint8_t*>(&can501.pedal_pot_sub_v);//ホストPCからのペダル指令値のsub実際値
			...
			tmp_ptr = reinterpret_cast<uint8_t*>(&can501.pedal_pot_main_v);//ホストPCからのペダル指令値のmain実際値
...
//					steering_can_value_left_to_tire_rad_slope_ + steering_can_value_left_to_tire_rad_intercept_;
			...
			//autowareに渡すステア情報をpublish(MPCなどで使用)
