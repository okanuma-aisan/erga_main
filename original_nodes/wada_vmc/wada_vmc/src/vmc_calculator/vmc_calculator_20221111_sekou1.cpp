...
//pedal_input_brake_max: 1516 #ブレーキ最大値
//pedal_input accel_min: 700 #アクセル全開値
...
namespace wada_vmc
{
	...
	private: // autowareからのcallback
		...
		{
			wada_vmc_msgs::msg::Can501_20221111 can501 = joy_receive_info_.get501(0);
			double scale_factor = (can501.pedal_pot_sub_v <= 1024) ? car_info_.accel_scale_factor_ : car_info_.brake_scale_factor_;
	...
	private: // その他の関数
		...
		void publishID100()
		{
			...
			// if(joy_receive_info_.get501Size() != 0 && joy_receive_info_.get502Size() != 0)
			// if(joy_receive_info_.get501Size() != 0)
			{
				// const wada_vmc_msgs::msg::Can501_20221111 can501 = joy_receive_info_.get501(0);
				// const wada_vmc_msgs::msg::Can502_20221111 can502 = joy_receive_info_.get502(0);
				...
				// ペダルコマンド
				if (joy_send_info_.getOperationAutoware() == true)
				{
					/*if(joy_receive_info_.get501(0).clutch == true)
					{
						int16_t pedal_cmd = car_info_.pedal_input_center_;//calculatJoyboardPedalCmd();
						can100->pedal_cmd = pedal_cmd;
						can100->pedal_cmd = std::max(std::min(can100->pedal_cmd, car_info_.pedal_input_brake_max_), car_info_.pedal_input_accel_min_);
					}
					else can100->pedal_cmd = car_info_.pedal_input_center_;
					joy_send_info_.addPedalCommand(can100->pedal_cmd, ros_nowtime);*/
					can100->pedal_cmd = joy_send_info_.getPedalCommand(0);
				...
				/*if(joy_send_info_.getOperationAutoware() == true)
				{
					if(joy_receive_info_.get502(0).clutch == true)
					{
						int16_t steer_cmd = car_info_.steer_input_center_;//calculatJoyboardSteerCmd();
						//steer_cmd = std::max(steer_cmd, (int16_t)0);
						can100->handle_cmd = steer_cmd;
						can100->handle_cmd = std::max(std::min(can100->handle_cmd, car_info_.steer_input_left_max_), car_info_.steer_input_right_min_);
					}
					else can100->handle_cmd = car_info_.steer_input_center_;
					joy_send_info_.addSteerCommand(can100->handle_cmd, ros_nowtime);
				}
				else
				{
					uint16_t steer_cmd = joy_send_info_.getSteerCommand(0);
					can100->handle_cmd = (steer_cmd == JoyBoardSendInfo::STEER_COMMAND_INVALID) ? car_info_.steer_input_center_ : steer_cmd;
				}*/
#if 0	// okanuma				
				if (joy_receive_info_.get502(0).clutch == false) // ステアクラッチが切れている場合は、クラッチが入った直後を見越して現在のステアポテンション値を前回のコマンド値として入れる
				{
					prev_steer_cmd_ = joy_receive_info_.get502(0).handle_pot_max;
					can100->handle_cmd = car_info_.steer_input_center_;
				}
				else
#endif				
				{
					/*int16_t cmd_diff = joy_send_info_.getSteerCommand(0) - prev_steer_cmd_;
					if (cmd_diff > 0)
					{
						int16_t new_cmd = prev_steer_cmd_ + 10;
						if (new_cmd > joy_send_info_.getSteerCommand(0))
							new_cmd = joy_send_info_.getSteerCommand(0);
						can100->handle_cmd = new_cmd;
					}
					else if (cmd_diff < 0)
					{
						int16_t new_cmd = prev_steer_cmd_ - 10;
					...
					prev_steer_cmd_ = joy_send_info_.getSteerCommand(0);
					can100->handle_cmd = joy_send_info_.getSteerCommand(0);*/
					can100->handle_cmd = joy_send_info_.getSteerCommand(0);
					...
					tmp.data = joy_send_info_.getSteerCommand(0);
					pub_tmp_int16_->publish(tmp);
				}
				...
				// フェイルセーフ
				can100->handle_failsafe = joy_send_info_.getSteerFailsafe();
				can100->pedal_failsafe = joy_send_info_.getPedalFailsafe();
	...
	public:
			...
			: rclcpp::Node("wada_vmc_calculation", node_option), joy_receive_info_(this->now()), joy_send_info_(this->now()), life_cycle_(1, 0), prev_steer_cmd_(1024)
		{
			...
			this->declare_parameter<int>("pedal_input_center", 0);
			...
			car_info_.accel_scale_factor_= get_parameter("accel_scale_factor").as_double();
			car_info_.brake_scale_factor_ = get_parameter("brake_scale_factor").as_double();
			...
			RCLCPP_INFO(this->get_logger(), "accel_scale_factor,%lf", car_info_.accel_scale_factor_);
			RCLCPP_INFO(this->get_logger(), "brake_scale_factor,%lf", car_info_.brake_scale_factor_);
