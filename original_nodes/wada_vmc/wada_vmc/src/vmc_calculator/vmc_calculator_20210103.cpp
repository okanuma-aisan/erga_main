...
namespace wada_vmc
{
	...
	private://autowareからのsubscriber
		...
		rclcpp::Subscription<auto_permission_msgs::msg::AutoPermission>::SharedPtr sub_auto_permission_;//自動運転のパーミッション情報
	...
	private://joyボード受信用callback
		...
		{
			//ステアクラッチがoffからonになったらフェイルセーフのライトを消す
			if(joy_receive_info_.get502Size() >= 1)
			{
				if(message502->clutch == true && joy_receive_info_.get502(0).clutch == false)
					joy_send_info_.setFailsafeLight(false);
			}
		...
		{
			//ペダルクラッチがoffからonになったらフェイルセーフのライトを消す
			if(joy_receive_info_.get503Size() >= 1)
			{
				if(message503->clutch == true && joy_receive_info_.get503(0).clutch == false)
					joy_send_info_.setFailsafeLight(false);
			}
	...
	private://autowareからのcallback
		...
		void callbackAutoPermission(const auto_permission_msgs::msg::AutoPermission::ConstSharedPtr perm)
		{
			if(perm->steer_permission != auto_permission_msgs::msg::AutoPermission::PERM_OK)
			{
				...
				joy_send_info_.setFailsafeLight(true);
			}
			else joy_send_info_.setSteerFailSafe(false);
			...
			else joy_send_info_.setPedalFailSafe(false);
		}

		//AUTOWAREからの自動運転炬可否フラグ
		...
		/*void callbackAutowareState(const autoware_adapi_v1_msgs::msg::OperationModeState autoware_state)
		{
			if (autoware_state.mode != autoware_adapi_v1_msgs::msg::OperationModeState::AUTONOMOUS)
			{
				...
				joy_send_info_.setPedalFailSafe(false);
				joy_send_info_.setFailsafeLight(true);
			}
			else
			{
	...
	private://joyボード制御用サービス
		...
		{
			joy_send_info_.setFailsafeLight(req->data);
			res->success = joy_send_info_.getFailsafeLight();
			res->message = "";
		}
	...
	private://その他の関数
		...
		void publishID100()
		{
			...
			//if(joy_receive_info_.get502Size() != 0 && joy_receive_info_.get503Size() != 0)
			{
				...
				//ステア操舵指令
				int16_t steer_cmd = joy_send_info_.getSteerCommand(0);
				can100->steer_command = (steer_cmd == JoyBoardSendInfo::STEER_COMMAND_INVALID) ? 0 : steer_cmd;
				...
				can100->pedal_command = (pedal_cmd == JoyBoardSendInfo::PEDAL_COMMAND_INVALID) ? 0 : pedal_cmd;
				...
				//ウィンカー
				switch (joy_send_info_.getBlinkerTrigger())
				{
				...
				// 自動可否
				can100->handle_auto_permission = joy_send_info_.getSteerFailSafe();
				...
				can100->pedal_auto_permission = joy_send_info_.getPedalFailSafe();
				...
				// フェイルセーフ
				can100->fail_safe_lighting = joy_send_info_.getFailsafeLight();
				...
				bool pedal_control = (joy_receive_info_.get503(0).clutch == true && joy_receive_info_.get501(0).drive_mode == wada_vmc_msgs::msg::Can501_20210103::DRIVE_AUTO);
				bool handle_control = (joy_receive_info_.get502(0).clutch == true && joy_receive_info_.get501(0).steer_mode == wada_vmc_msgs::msg::Can501_20210103::STEER_AUTO);
	...
	public:
		VmcCalculator20210103(const rclcpp::NodeOptions &node_option)
			...
			, life_cycle_(1, 0)
		{
			this->declare_parameter<int>("steer_input_left_max", 15000);
			this->declare_parameter<int>("steer_input_right_min", -15000);
			this->declare_parameter<int>("pedal_input_accel_max", 500);
			this->declare_parameter<int>("pedal_input_brake_min", -350);
			...
			car_info_.pedal_input_accel_max_ = static_cast<int16_t>(this->get_parameter("pedal_input_accel_max").as_int());
			car_info_.pedal_input_brake_min_ = static_cast<int16_t>(this->get_parameter("pedal_input_brake_min").as_int());
			...
			sub_auto_permission_ = this->create_subscription<auto_permission_msgs::msg::AutoPermission>(
				"auto_permission", rclcpp::QoS(1),
				std::bind(&VmcCalculator20210103::callbackAutoPermission, this, std::placeholders::_1));
