...
private:
	...
	rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr sub_kinematic_stat_;
	...
	void callbackKinematicStat(const nav_msgs::msg::Odometry::ConstSharedPtr stat)
	{
		...
		buf[0] = stat->pose.pose.position.x;
		buf[1] = stat->pose.pose.position.y;
		buf[2] = stat->pose.pose.position.z;
		buf[3] = stat->pose.pose.orientation.x;
		buf[4] = stat->pose.pose.orientation.y;
		buf[5] = stat->pose.pose.orientation.z;
		buf[6] = stat->pose.pose.orientation.w;
		buf[7] = stat->twist.twist.linear.x;
		buf[8] = stat->twist.twist.linear.y;
		buf[9] = stat->twist.twist.linear.z;
		buf[10] = stat->twist.twist.angular.x;
		buf[11] = stat->twist.twist.angular.y;
		buf[12] = stat->twist.twist.angular.z;
...
public:
		...
		, sock_(-1)
	{
		...
		sub_kinematic_stat_ = this->create_subscription<nav_msgs::msg::Odometry>(
			"input_kinematic_stat", rclcpp::QoS(1), std::bind(&PoseUdpPub::callbackKinematicStat, this, std::placeholders::_1));
