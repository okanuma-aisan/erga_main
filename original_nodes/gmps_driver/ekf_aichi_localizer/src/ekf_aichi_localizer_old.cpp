...
private:
	...
	double sign_reverse_; //バックしてるかどうか 1 or -1
	double x_gnss_prev_; //debug ギアが取れれば不要
	double y_gnss_prev_; //debug ギアが取れれば不要
	...
	void time_update()
	{
		...
		Fk_(IDX::X, IDX::KV)    =     dx;
		Fk_(IDX::X, IDX::KB)    = -kv*dy*beta;
		Fk_(IDX::Y, IDX::THETA) =  kv*dx;
		Fk_(IDX::Y, IDX::KV)    =     dy;
		Fk_(IDX::Y, IDX::KB)    =  kv*dx*beta;
