...
/* debug comment
covarianceをそのまま使う？明示的にcallbackを分けて固定値を使う？
使用可否の判断もここで行う？

twist, poseの遅延をどうする？ smoother入れる？　いや入れんやろ
じゃあ遅延補間をするのか？ gmpsは遅延補間をしてから送信している
subしたらflagを立てて、各フラグが立っていればmeasurement updateを連打する
autowareのようにqueueで管理する？

ndt+ekfのやつから引っ張ってくれば、遅延は吸収してくれているので、そっちを使いましょう
*/
...
#include <gmps_msgs/msg/gmps_info.hpp>
...
private:
	...
	rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr pub_ekf_aichi_pose_nocov_;
	rclcpp::Publisher<gmps_msgs::msg::DebugEkfLog>::SharedPtr pub_debug_ekf_log_;
	rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pub_kinematic_stat_;//車両位置と車両速度情報
	rclcpp::Publisher<std_msgs::msg::Float64>::SharedPtr pub_ekf_aichi_beta_;//タイヤーの横滑り角
	rclcpp::Publisher<std_msgs::msg::String>::SharedPtr pub_select_localization_;//現在寄与が大きいlocalizer
	...
	rclcpp::Subscription<gmps_msgs::msg::GmpsInfo>::SharedPtr sub_gmps_info_;
	rclcpp::Subscription<tier4_debug_msgs::msg::Float32Stamped>::SharedPtr sub_control_lateral_error_;
	...
	rclcpp::TimerBase::SharedPtr reverse_timer_; //debug バックかどうか判定するだけのタイマー VCANからギア情報をとれば不要
	...
	bool f_ndt_receive_;//ndtトピックsubフラグ
	bool f_gnss_receive_;//gnssトピックsubフラグ
	bool f_gmps_receive_;//gmpsトピックsubフラグ
	bool f_ndt_init_;     // ndt初期化フラグ

	/*time stamp*/
	std::vector<double> ndt_stamp_;//NDTのタイムスタンプ
	std::vector<double> gnss_stamp_;//GNSSのタイムスタンプ
	std::vector<double> gmps_stamp_;//GMPSのタイムスタンプ
	...
	gmps_msgs::msg::GmpsInfo gmps_info_;//GMPSマーカー情報
	float control_lateral_error_;//センターラインからの距離
	...
	gmps_msgs::msg::DebugEkfLog log_msg;
	...
	void callbackNdtPose(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
	{
		...
		tf2::Quaternion quandt(msg->pose.pose.orientation.x, msg->pose.pose.orientation.y,
			msg->pose.pose.orientation.z, msg->pose.pose.orientation.w);
		double yawndt;
		tf2::Matrix3x3(quandt).getRPY(roll_ndt_, pitch_ndt_, yawndt);

		ndt_stamp_.push_back(this->now().seconds());
	}

	
	void callbackGnssPose(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
	{
		f_gnss_receive_ = true;
		check_gnss();
		...
		tf2::Quaternion quagnss(msg->pose.pose.orientation.x, msg->pose.pose.orientation.y,
			msg->pose.pose.orientation.z, msg->pose.pose.orientation.w);
		double yawgnss;
		tf2::Matrix3x3(quagnss).getRPY(roll_gnss_, pitch_gnss_, yawgnss);

		gnss_stamp_.push_back(this->now().seconds());
	}

	void callbackGmpsPose(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
	{
		f_gmps_receive_ = true;
		check_gmps();
		...
		//debug enableとかの確認
		RCLCPP_INFO(this->get_logger(), "enable_gmps=%d, gmps_ready=%d, ndt_init=%d",enable_gmps_, f_gmps_ready_, f_ndt_init_);
		//debug end
		...
		gmps_stamp_.push_back(this->now().seconds());
	}

	void callbackGmpsInfo(const gmps_msgs::msg::GmpsInfo::ConstSharedPtr info)
	{
		gmps_info_ = *info;
	}

	void callbackControlLateralError(const tier4_debug_msgs::msg::Float32Stamped::ConstSharedPtr msg)
	{
		control_lateral_error_ = msg->data;
	}
	...
	void publish_pose()
	{
        ...
        //q.setRPY(0.0, 0.0, xk_(IDX::THETA) + beta_);//publish yaw+beta
		...
		geometry_msgs::msg::PoseStamped msg_pose_nocov;
		msg_pose_nocov.header = ekf_aichi_pose_.header;
		msg_pose_nocov.pose = ekf_aichi_pose_.pose.pose;
		pub_ekf_aichi_pose_nocov_->publish(msg_pose_nocov);

		std_msgs::msg::Float64 msg_beta;
		msg_beta.data = beta_;
		pub_ekf_aichi_beta_->publish(msg_beta);
	...
	void publish_kinematic_stat()
	{
		...
		tf2::Quaternion q;
		if(enable_ndt_) q.setRPY(roll_ndt_, pitch_ndt_, xk_(IDX::THETA));
		if(enable_gnss_) q.setRPY(roll_gnss_, pitch_gnss_, xk_(IDX::THETA));
		else q.setRPY(0.0, 0.0, xk_(IDX::THETA));
        //q.setRPY(0.0, 0.0, xk_(IDX::THETA) + beta_); //publish yaw+beta
	...
	{
		log_msg.in_ndt_x = z0_(0);
		log_msg.in_ndt_y = z0_(1);
		log_msg.in_ndt_z = z_ndt_;
		log_msg.in_ndt_yaw = z0_(2);
		log_msg.in_gnss_x = z1_(0);
		log_msg.in_gnss_y = z1_(1);
		log_msg.in_gnss_z = z_gnss_;
		log_msg.in_gnss_yaw = z1_(2);
		log_msg.in_gmps_x = z2_(0);
		log_msg.in_gmps_y = z2_(1);
		log_msg.in_gmps_yaw = z2_(2);
		log_msg.ekf_x = xk_(IDX::X);
		log_msg.ekf_y = xk_(IDX::Y);
		log_msg.ekf_z = z_ekf_aichi_;
		log_msg.ekf_yaw = xk_(IDX::THETA);
		log_msg.ekf_kv = xk_(IDX::KV);
		log_msg.ekf_gb = xk_(IDX::GB);
		log_msg.ekf_kbz = xk_(IDX::KB);
		log_msg.ekf_beta = beta_;
		log_msg.in_vx_raw = uk_(IDX_U::V);
		log_msg.in_delta_raw = uk_(IDX_U::Delta);
		log_msg.in_gamma_raw = uk_(IDX_U::Gamma);
		log_msg.f_ndt_update = f_ndt_update_;
		log_msg.f_gnss_update = f_gnss_update_;
		log_msg.f_gmps_update = f_gmps_update_;
		log_msg.f_ndt_ready = f_ndt_ready_;
		log_msg.f_gnss_ready = f_gnss_ready_;
		log_msg.f_gmps_ready = f_gmps_ready_;
		log_msg.in_ndt_tp = ndt_tp_;
		log_msg.in_ndt_nvtl = ndt_nvtl_;
		log_msg.in_gnss_fix_quality = gnss_fix_quality_;
		log_msg.in_gnss_hdop = gnss_hdop_;
		log_msg.dist_euclid_ndt = dist_euclid_ndt_;
		log_msg.dist_mahalanobis_ndt = dist_mahalanobis_ndt_;
		log_msg.dist_euclid_gnss = dist_euclid_gnss_;
		log_msg.dist_mahalanobis_gnss = dist_mahalanobis_gnss_;
		log_msg.dist_euclid_gmps = dist_euclid_gmps_;
		log_msg.dist_mahalanobis_gmps = dist_mahalanobis_gmps_;
		log_msg.control_lateral_error = control_lateral_error_;
		log_msg.gmps_info = gmps_info_;
		log_msg.f_ndt_receive = f_ndt_receive_;
		log_msg.f_gnss_receive = f_gnss_receive_;
		log_msg.f_gmps_receive = f_gmps_receive_;
	...
	void contribution()
	{
		...
		constexpr double ndt_timediff_th = 0.5;
		constexpr double gnss_timediff_th = 0.5;
		constexpr double gmps_timediff_th = 1.5;

		for(int i=ndt_stamp_.size()-1; i>=0; i--)
		{
			if(nowtime - ndt_stamp_[i] <= ndt_timediff_th && i != ndt_stamp_.size()-1)
			{
				ndt_stamp_.erase(ndt_stamp_.begin()+i, ndt_stamp_.end());
				break;
			}
		}
		for(int i=gnss_stamp_.size()-1; i>=0; i--)
		{
			if(nowtime - gnss_stamp_[i] <= gnss_timediff_th && i != gnss_stamp_.size()-1)
			{
				gnss_stamp_.erase(gnss_stamp_.begin()+i, gnss_stamp_.end());
				break;
			}
		}
		for(int i=gmps_stamp_.size()-1; i>=0; i--)
		{
			if(nowtime - gmps_stamp_[i] <= gmps_timediff_th && i != gmps_stamp_.size()-1)
			{
				gmps_stamp_.erase(gmps_stamp_.begin()+i, gmps_stamp_.end());
				break;
			}
		}
		/*for(int i=ndt_stamp_.size()-1; i>=0; i--)
			if(nowtime - ndt_stamp_[i] > ndt_timediff_th) ndt_stamp_.erase(ndt_stamp_.end());
		for(int i=gnss_stamp_.size()-1; i>=0; i--)
			if(nowtime - gnss_stamp_[i] > gnss_timediff_th) gnss_stamp_.erase(gnss_stamp_.end());
		for(int i=gmps_stamp_.size()-1; i>=0; i--)
			if(nowtime - gmps_stamp_[i] > gmps_timediff_th) gmps_stamp_.erase(gmps_stamp_.end());*/

		std_msgs::msg::String msg_conv_str;
		double ndt_cont = (gmps_stamp_.size() != 0) ? ndt_stamp_.size() * sigma_x_ndt_ * sigma_x_ndt_ : 0;
		double gnss_cont = (gmps_stamp_.size() != 0) ? gnss_stamp_.size() * sigma_x_gnss_ * sigma_x_gnss_ : 0;
		msg_conv_str.data = (ndt_cont > gnss_cont) ? "NDT" : "GNSS";
		double gmps_cont = (gmps_stamp_.size() != 0) ? gmps_stamp_.size() * sigma_x_gmps_ * sigma_x_gmps_ : 0;
		if(gmps_cont != 0) msg_conv_str.data = "GMPS";
		std::stringstream ss;
		ss << msg_conv_str.data << "," << ndt_cont << "," << gnss_cont << "," << gmps_cont;
		msg_conv_str.data = ss.str();
		pub_select_localization_->publish(msg_conv_str);
	}
	...
	void callbackTimer()
	{
		...
		{
			if (dist_mahalanobis_ndt_ < max_dist_)
			{
		...
		{
			if (dist_mahalanobis_gnss_ < max_dist_)
			{
		...
		{
			if (dist_mahalanobis_gmps_ < max_dist_)
			{
				...
				//debug 走行距離のリセットが距離の判定後でないとできない
        ...
        f_ndt_receive_ = false;
        f_gnss_receive_ = false;
        f_gmps_receive_ = false;
...
public:
		...
		, f_gmps_receive_(false)
	{
		...
		pub_ekf_aichi_pose_nocov_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("out_ekf_aichi_pose_nocov", rclcpp::QoS(1));
		pub_debug_ekf_log_ = this->create_publisher<gmps_msgs::msg::DebugEkfLog>("debug_ekf_log", rclcpp::QoS(1));
		pub_kinematic_stat_ = this->create_publisher<nav_msgs::msg::Odometry>("out_odom_name", rclcpp::QoS(1));
		pub_ekf_aichi_beta_ = this->create_publisher<std_msgs::msg::Float64>("beta", rclcpp::QoS(1));
		pub_select_localization_ = this->create_publisher<std_msgs::msg::String>("select_localization", rclcpp::QoS(1));
		...
		sub_gmps_info_ = this->create_subscription<gmps_msgs::msg::GmpsInfo>(
			"gmps_info", 10, std::bind(&EKFAichiLocalizer::callbackGmpsInfo, this, std::placeholders::_1));
		sub_control_lateral_error_ = this->create_subscription<tier4_debug_msgs::msg::Float32Stamped>(
			"/lateral_error_publisher/control_lateral_error", 10, std::bind(&EKFAichiLocalizer::callbackControlLateralError, this, std::placeholders::_1));
