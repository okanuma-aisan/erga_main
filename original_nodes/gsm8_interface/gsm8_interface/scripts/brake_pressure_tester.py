...
class BrakeTester(Node):
    def __init__(self):
        super().__init__("brake_pressure_tester")
        self.pub = self.create_publisher(Float32Stamped, "/vehicle/tester/brake_pressure", 1)

    def run(self):
        while rclpy.ok():
            value = float(
                input("target brake [" + str(MIN_BRAKE) + " ~ " + str(MAX_BRAKE) + " MPa] > ")
            )
