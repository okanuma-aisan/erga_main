...
class ThrottleTester(Node):
    def __init__(self):
        super().__init__("throttle_tester")
        self.pub = self.create_publisher(Float32Stamped, "/vehicle/tester/throttle", 1)
...
def main(args=None):
    ...
    throttle_tester = ThrottleTester()
    throttle_tester.run()

    throttle_tester.destroy_node()
    rclpy.shutdown()
