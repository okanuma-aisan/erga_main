...
class EPSVoltagePublisher(Node):
    def __init__(self):
        super().__init__("eps_voltage_publisher")
        self.target_eps_voltage = 0.0

        self.sub = self.create_subscription(
            Float32Stamped, "/vehicle/tester/eps_voltage", self.on_eps_voltage, 1
        )
    ...
    def on_eps_voltage(self, msg):
        self.target_eps_voltage = msg.data
        print(f"Set target EPS voltage: {self.target_eps_voltage}")
    ...
    def on_timer(self):
        ...
        msg_actuation_cmd.actuation.steer_cmd = self.target_eps_voltage
        ...
        print(f"publish VehicleCommand with eps voltage value: {self.target_eps_voltage}")


def main(args=None):
    rclpy.init(args=args)

    eps_voltage_publisher = EPSVoltagePublisher()

    rclpy.spin(eps_voltage_publisher)

    eps_voltage_publisher.destroy_node()
    rclpy.shutdown()
